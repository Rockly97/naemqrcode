...
public class DetailItemAdapter extends RecyclerView.Adapter {
    ...
    private void dealOnPaste(DetailItemViewHolder holder) {
        ...
        if (mClipboardManager.hasPrimaryClip()) {
            ...
            ContactBean mBean = (ContactBean) clipData.getItemAt(0).getIntent().getSerializableExtra("mBean");
            int index = holder.mValueEditText.getSelectionStart();

            Editable editable = holder.mValueEditText.getEditableText();
            String tag = holder.mTagEditText.getText().toString();
            try {
                if(tag.equals("电话")){
                    editable.insert(index, mBean.getPhone().get(0).getValue());
                }
                if(tag.equals("邮箱")){
                    editable.insert(index, mBean.getEmail().get(0).getValue());
                }
                if(tag.equals("微信")){
                    editable.insert(index, mBean.getWechat().get(0).getValue());
                }
                if(tag.equals("其他")){
                    editable.insert(index, mBean.getOther().get(0).getValue());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            holder.mValueEditText.addTextChangedListener(new TextWatcher() {
                @Override
                public void beforeTextChanged(CharSequence s, int start, int count, int after) {
    ...
    private void dealOnPaste(DetailHeadViewHolder holder, String tag) {
        ...
        if (mClipboardManager.hasPrimaryClip()) {
            ...
            if (tag.equals("Name")) {
                ...
                editable.insert(index, mBean.getName());
//                editable.insert(index, clipData.getItemAt(0).getText());
                ...
                editable.insert(index, mBean.getRemark());
                //editable.insert(index, clipData.getItemAt(0).getText());
            }
        }
    }
