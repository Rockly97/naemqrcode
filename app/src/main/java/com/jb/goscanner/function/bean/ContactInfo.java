...
public class ContactInfo implements Serializable {
    ...
    // 用于将 RecyclerView 的内容组装成 ContactInfo (items包含group类型，组装成的info是对象没有group类型）
    public static ContactInfo combineToContactInfo(List<DetailItem> items, String contactId) {
        ContactInfo info = new ContactInfo();
        if (items != null && items.get(0).getContactId().equals(DetailItem.GROUP_HEAD)) {
    ...
    public static String parseToString(ContactInfo contact) {
        ...
        try {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                jsonContact.put("detail" + i, item.parseToJsonString());
                jsonArray.put(i,jsonContact);
            }
    ...
    public static ContactInfo parseToContactInfo(String jsonString) {
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                JSONObject detail = new JSONObject(object.getString("detail" + i));
                DetailItem item = DetailItem.parseToDetailItem(detail.getString("detail"));
//                DetailItem item = DetailItem.parseToDetailItem((String) listObject.get("detail" + i));
                items.add(item);
            }
